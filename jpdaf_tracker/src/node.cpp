...
//        ROS_INFO("Real measured time step: %f", ros::Time::now().toSec() - last_timestamp_ros_time_now_debug);
        ...
        for(uint t=0; t<tracks_.size(); t++)
        {
            ...
            cout << "betas of track " << t << ": "; for(uint b=0; b<beta.size(); b++){cout << beta[b] << ", ";} cout << "; beta_0 = " << beta_0 << endl;*/
//-----
...
//            ROS_INFO("Updating track %d", tracks_[t].getId());
...
/*std::vector<double> Node::compute_beta(int track_nb, std::vector<Eigen::MatrixXf> hypothesis_matrices, std::vector<double> hypothesis_probabilities)
{
    ...
    for(int i=0; i<hypothesis_matrices[0].rows(); i++) // for each measurement
    {
        ...
        {
            if(hypothesis_matrices[h](i, track_nb + 1) == 1)
            {
...
std::vector<double> Node::compute_alphas_0(std::vector<Eigen::MatrixXf> hypothesis_mats, std::vector<double> hypothesis_probs)
{
    ...
    for(uint h=0; h<hypothesis_mats.size(); h++)
    {
        ...
        {
            if(detected_tracks(i-1) == 0) //MIGHT BE PROBLEM HERE
            {
...
std::vector<double> Node::compute_betas_0(std::vector<Eigen::MatrixXf> hypothesis_mats, std::vector<double> hypothesis_probs)
{
...
void Node::compute_timescaled_orientation_change_flush_pose(double detection_time_stamp)
{
    ...
    //TODO check buffer size and lateness checks and adapt if too strict!!!
